...
function BankPanel.new(parent, onDeposit, onWithdraw)
  ...
  balanceLabel.TextColor3 = Color3.fromRGB(200, 180, 80)
  ...
  depositBtn.TextColor3 = Color3.fromRGB(140, 200, 80)
  ...
  withdrawBtn.TextColor3 = Color3.fromRGB(140, 200, 80)
  ...
  local function parseAmount(isDeposit)
    ...
    if raw == "A" and latestState then
      if isDeposit then return latestState.cash else return latestState.bankBalance end
    end
  ...
  depositBtn.Activated:Connect(function()
    ...
    if qty > 0 then onDeposit(qty); amountBox.Text = "" end
  end)
  withdrawBtn.Activated:Connect(function()
    local qty = parseAmount(false)
    if qty > 0 then onWithdraw(qty); amountBox.Text = "" end
  end)
