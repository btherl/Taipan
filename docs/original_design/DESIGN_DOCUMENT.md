# Taipan! -- Roblox Single-Player Game Design Document (Version 3.0)

## Change Log

### v3.0 (2026-03-16) — Second Review Corrections

1. **Ship status percentage uses rounding, not truncation** — Line 313: `WW = 100 - INT(DM/SC * 100 + .5)`. Updated formula everywhere to reflect rounding.
2. **Ship repair damage calculation uses rounding; "All" cost includes +1** — Line 1160: `WW = INT(W / BR + .5)` uses rounding. Line 1150: "A" (All) sets `W = BR * DM + 1` — the +1 ensures rounding fully covers all damage.
3. **Opium seizure explicitly frees hold space** — Line 1910: `MW = MW + ST(2,1)` restores hold space before zeroing opium.
4. **Wu debt repayment skip condition documented** — Line 1360: `IF DW = 0 OR CA = 0 THEN 1400` skips repayment prompt if no debt or no cash. Borrow prompt (line 1400) always appears.
5. **Li Yuen warning has 3-in-4 probability** — Line 2330: `FN_R(4)` is truthy (non-zero) 3-in-4 times. Warning fires with 75% chance, not always.
6. **Cargo theft adjusts WS warehouse-used counter** — Line 2030: `WS = WS - W + WW` updates warehouse tracking after each good is reduced.
7. **D=0 guard at game start** — Line 1010: `IF D <> 0 THEN ...` — entire arrival processing (interest, debt, time advance) is skipped when D=0 (initial game start).
8. **Non-HK ports skip directly to line 1500** — Line 1030: `IF LO <> 1 THEN 1500` — all Hong Kong events (Li Yuen offer, repair, Wu) skipped for other ports.
9. **Menu options vary by location and wealth** — Lines 2522-2528: BSQ at non-HK, BSQTV at HK below 1M net worth, BSQTVR at HK at or above 1M. Documented full menu structure.
10. **LI% confirmed as dead code** — Nothing ever sets LI% to a non-zero initial value. The increment path (`IF LI = 0 AND LI% > 0`) can never trigger.
11. **OK=16 for millionaire retirement** — Line 2695: `OK = 16` distinguishes successful retirement from other game-over conditions.
12. **Combat targeting retry on empty slot** — Lines 5320-5330: random slot pick retries if slot is empty, with SN=0 guard before the loop.
13. **Combat ship spawning algorithm documented** — Lines 5710-5730: spawn-on-vacancy algorithm fills empty grid slots with new ships from the waiting pool.
14. **Per-player state maps in all code examples** — Replaced global `currentPhase`/`gameState` with per-player maps (`playerStates[player]`).
15. **GetGameState RemoteFunction replaced with event-based approach** — RemoteFunction vulnerable to client-side yield attacks; replaced with RequestStateUpdate RemoteEvent.
16. **DataStore retry logic added** — Exponential backoff with 3 retries for all DataStore calls.
17. **DataStore budget consideration documented** — Rate limits and save-frequency recommendations added.
18. **Mid-combat save recommendation** — Recommended against mid-combat saves for v1; save at combat resolution instead.
...
- **Win (Retirement):** The player can retire at any time while in Hong Kong. If their net worth (cash + bank - debt) reaches 1,000,000 or more, they achieve "Millionaire" status (OK=16) and can retire with honor. Below that threshold, they can still retire but with a lower score.
- **Loss (Death/Bankruptcy):** The player dies if their ship sinks (0% seaworthiness from combat damage or storms, OK=0), or if they refuse Elder Brother Wu's emergency loan when completely broke (game over).
...
#### Menu Options by Location and Wealth

The available menu options change depending on the player's location and net worth. This is critical for UI implementation.

**Menu structure (lines 2522-2528):**

| Condition | Options | Keys |
|-----------|---------|------|
| Not in Hong Kong | Buy, Sell, Quit | "BSQ" |
| In HK, net worth < 1,000,000 | Buy, Sell, Quit, Transfer, Visit Wu | "BSQTV" |
| In HK, net worth >= 1,000,000 | Buy, Sell, Quit, Transfer, Visit Wu, Retire | "BSQTVR" |

**Net worth for this check** is calculated at line 2520 as `I = CA + BA - DW` (cash + bank - debt).

- **Buy / Sell:** Available at all ports. Buy/sell goods from the ship hold at current market prices.
- **Quit:** Depart to another port (presents destination selection 1-7).
- **Transfer:** Hong Kong only. Move cargo between ship hold and warehouse (bidirectional for each good, checking warehouse capacity).
- **Visit Wu:** Hong Kong only. Access Elder Brother Wu's services (debt repayment, borrowing, bank deposit/withdraw).
- **Retire:** Hong Kong only, net worth >= 1,000,000. End the game with the retirement score screen.

The UI must dynamically show/hide buttons based on these conditions and recalculate net worth each time the menu is displayed.
...
- Ship status: **`100 - INT(DM/SC * 100 + 0.5)`** — uses rounding, not truncation (line 313: the `+ .5` inside INT produces banker's rounding behavior). Clamped to minimum 0 via `WW = WW * (WW > 0)`.
...
When visiting Wu in Hong Kong (line 1310 onward), the flow is:

1. **Bankruptcy check (line 1320-1350):** If player has no cash, no bank, no cargo anywhere, and no guns, trigger the emergency loan flow (see below).
2. **Debt repayment (line 1360):** Prompt to repay debt. **Skipped if `DW = 0` (no debt) OR `CA = 0` (no cash)** — line 1360: `IF DW = 0 OR CA = 0 THEN 1400`. Both must be non-zero for the repayment prompt to appear.
3. **Borrow (line 1400):** Prompt to borrow from Wu. **Always appears regardless of debt or cash status** — the code falls through to line 1400 unconditionally after the repayment section.
...
**`LI%` Counter (confirmed dead code):**
- The `LI%` variable is intended to increment when protection has lapsed (`LI = 0`) AND `LI% > 0` (line 2310)
- When `LI%` exceeds 4, it resets to 0
- **However, `LI%` is initialized to 0 at game start (line 10, as part of the CLEAR and variable initialization) and nothing in the BASIC listing ever sets it to a non-zero value initially.** The increment logic requires `LI% > 0` to trigger, so `LI%` can never become non-zero — the initialization to `LI% = 1` on first protection lapse is missing from the BASIC source. This is confirmed dead code, likely a bug or incomplete feature from development.

#### Mugging / Seizure Events

1. **Opium seizure** (line 1900 — when **ship hold** has opium `ST(2,1) != 0`, not in Hong Kong `LO != 1`, 1-in-18 chance):
   - **Frees hold space first:** `MW = MW + ST(2,1)` — the opium's hold space is restored to available capacity (line 1910)
   - Then confiscates all opium from **ship hold** (`ST(2,1) = 0`)
...
2. **Cargo theft** (lines 2000-2030 — when **warehouse** has goods `ST(1,J) total != 0`, 1-in-50 chance):
   ...
   - **`WS` (warehouse-used counter) is updated for each good:** `WS = WS - W + WW` where W is the original amount and WW is the remaining amount (line 2030). This keeps warehouse capacity tracking accurate after theft.
...
- Each shot targets a random on-screen enemy ship via a **slot-retry algorithm** (lines 5320-5330):
  - Pick a random grid slot: `I = FN_R(10)`
  - If the slot is empty (`AM%(I,0) = 0`), retry (loop back to pick another slot)
  - **Guard:** The gun-fire loop is only entered if `SN > 0` (line 5310: `IF SN = 0 THEN 5340`). In the Lua implementation, always verify at least one ship is visible before entering the targeting loop to prevent infinite retries.
...
- **Ship destroyed (0% seaworthiness, OK=0):** Game over — jumps to score screen via line 2698.
- **Millionaire retirement (OK=16):** Player retired as millionaire — jumps to score screen.
...
The BASIC.txt version displays up to 10 enemy ships on screen in a 5x2 grid. Each has independent HP. When more than 10 enemies exist, new ships are spawned into empty slots as visible ones are sunk.

**Grid Position Formula (line 5880):**
- Slot `I` (0-9) maps to screen coordinates:
  - `X = (I mod 5) * 8 + 1`
  - `Y = INT(I / 5) * 6 + 7`
- This produces a 5-column, 2-row grid with 8-unit horizontal spacing and 6-unit vertical spacing.

**Spawn-on-Vacancy Algorithm (lines 5710-5730):**

When enemy ships are displayed or refreshed, each empty grid slot (`AM%(I,0) = 0`) is checked. If there are waiting ships (`SA > 0`), a new ship spawns into the empty slot:

```
for I = 0 to 9:
    if AM%(I,0) != 0 then continue  -- slot occupied
    SA = SA - 1                      -- one fewer waiting
    if SA < 0 then SA = 0; return    -- no more waiting ships
    AM%(I,0) = FN_R(EC) + 20        -- new ship HP
    AM%(I,1) = 0                     -- reset damage
    draw ship at grid position I
    SS = SS + 1                      -- increment on-screen count
```

The variable `SA` tracks how many enemies still need to appear, and `SS` tracks how many are currently on screen.
...
- Player is shown damage percentage: `INT(DM / SC * 100 + 0.5)` — uses rounding (line 1142)
- Full repair cost displayed: `BR * DM + 1` — **the +1 ensures that when rounding is applied to the repair amount, all damage is fully covered** (line 1145)
- Can pay for partial repair (any amount). "A" (All) pays for full repair: `W = BR * DM + 1`, capped by available cash (line 1150)
- **Damage repaired uses rounding:** `INT(amount_paid / cost_per_unit + 0.5)` (line 1160: `WW = INT(W / BR + .5)`)
- DM is then reduced by the repaired amount and clamped to minimum 0: `DM = INT(DM * (DM > 0))`
...
**Special case — Initial game start (D=0):**

At game start, `D` (destination) is 0. Line 1010 checks `IF D <> 0 THEN ...` — when D=0, the entire arrival processing block (bank interest, debt compounding, time advance, month/year updates) is **skipped**. This means the very first Hong Kong visit has:
- No time advance (TI stays at 1)
- No bank interest applied
- No debt compounding
- The player goes directly to the Hong Kong event sequence

This is a critical implementation detail: the ARRIVAL state must check whether D=0 and bypass time/financial processing if so.

**Non-HK port shortcut:**

Line 1030: `IF LO <> 1 THEN 1500` — when the player arrives at any port other than Hong Kong, the entire HK event block (Li Yuen offer, ship repair, Wu warning, Wu visit, Wu enforcers) is **skipped entirely**. The game jumps directly to ship upgrade checks (line 1500).

**Full arrival sequence (matching BASIC line numbers):**

| Order | BASIC Lines | Event | Interactive? | Notes |
|-------|-------------|-------|--------------|-------|
| 0 | 1010 | D=0 guard | No | If D=0 (game start), skip ALL of order 1-2 |
| 1 | 1010 | Time advance, interest, debt compound | No | Only when D != 0 |
| 2 | 1020 | January checks (EC, ED, base price drift) | No | Only if month > 12 |
| — | 1030 | Non-HK port skip | No | If LO != 1, jump to order 8 (line 1500) |
...
| 10 | 1900-1910 | Opium seizure | No | Ship hold opium seized (hold space freed); not in HK; 1-in-18 |
| 11 | 2000-2030 | Cargo theft | No | Warehouse goods stolen (WS updated); 1-in-50 |
| 12 | 2100 | Price calculation | No | Always runs |
| 13 | 2310 | Li Yuen protection lapse check | No | 1-in-20 chance protection expires |
| 14 | 2330 | Li Yuen warning message | No | If unprotected, not in HK, AND **3-in-4 chance** (FN_R(4) non-zero) |
| 15 | 2410-2450 | Price events (crash/boom) | No | 1-in-9 chance |
| 16 | 2500-2501 | Cash robbery | No | If cash > 25000, 1-in-20 |
| — | 2510+ | Player gains control (trading menu) | Yes | Menu options depend on location and wealth |
...
| Opium seizure | Arriving at port | 1/18 (if **ship hold** has opium) | Not Hong Kong | Ship hold opium (hold space freed) |
| Cash robbery | At port | 1/20 (if cash > 25,000) | Any port | Cash |
| Cargo theft | At port | 1/50 (if **warehouse** has cargo) | Any port | Warehouse goods (WS updated) |
...
| Li Yuen warning | At port | **3/4** (if unprotected and not in HK) | Not Hong Kong | — |
...
- **InputHandler** -- Captures player choices and sends RemoteEvent calls to server
...
export type GameState = {
    ...
    -- Location / Time
    currentPort: PortId,
    destination: PortId,      -- D: travel destination (0 at game start)
    month: number,            -- 1-12
    ...
    shipStatus: number,       -- percentage 0-100 (uses rounding: 100 - INT(DM/SC*100 + 0.5))
}
...
export type CombatState = {
    ...
    onScreenCount: number,    -- SS: currently visible ships
...
**State Definitions:**
- **ARRIVAL** — Non-interactive processing cascade upon reaching a port. Covers time advancement, interest/debt compounding, January checks, and all automatic events (opium seizure, cargo theft, price calculation, Li Yuen lapse, price events, robbery). **When D=0 (game start), time/financial processing is skipped.** When not in Hong Kong, HK-only events are skipped entirely. No player input required.
...
- **RETIREMENT** — Player has chosen to retire. If net worth >= 1,000,000, sets OK=16 (millionaire). Leads to the score screen.
...
Phase: NAMING --> PORT --> TRADING <--> PORT
                  ...
                  PORT               COMBAT / STORM
                                       ...
                                       ARRIVAL (non-interactive cascade)
                                          |  (D=0 at game start: skip time/financial)
                                          |  (non-HK: skip to ship upgrade checks)
                                          v
                                       ...
                                       BANKRUPTCY (if broke)
                                          |
                                          v
                                        PORT (menu varies by location/wealth)
                                          |
                                          v
                                     RETIREMENT (OK=16 if millionaire) / GAME_OVER (OK=0 if sunk)
```
...
-- Per-player state: each player has their own independent game state and phase
...
Players.PlayerAdded:Connect(function(player)
    local savedState = loadGame(player)
    playerStates[player] = {
        phase = savedState and savedState.phase or "NAMING",
        gameState = savedState and savedState.gameState or createNewGameState(),
        combatState = nil,
    }
end)

-- Cleanup state when player leaves
Players.PlayerRemoving:Connect(function(player)
    local pState = playerStates[player]
    if pState then
        saveGame(player, pState.gameState)
    end
...
PlayerActionEvent.OnServerEvent:Connect(function(player, action, data)
    ...
    if pState.phase == "PORT" then
        if action == "BUY" then
            handleBuy(player, pState, data.goodIndex, data.quantity)
        elseif action == "SELL" then
            handleSell(player, pState, data.goodIndex, data.quantity)
        elseif action == "TRANSFER" then
            handleTransfer(player, pState, data.goodIndex, data.quantity, data.direction)
        elseif action == "BANK_DEPOSIT" then
            handleBankDeposit(player, pState, data.amount)
        elseif action == "BANK_WITHDRAW" then
            handleBankWithdraw(player, pState, data.amount)
        elseif action == "REPAY_DEBT" then
            handleRepayDebt(player, pState, data.amount)
        elseif action == "BORROW" then
            handleBorrow(player, pState, data.amount)
        elseif action == "REPAIR" then
            handleRepair(player, pState, data.amount)
        elseif action == "QUIT_PORT" then
            handleSetSail(player, pState, data.destination)
        elseif action == "RETIRE" then
            handleRetire(player, pState)
        end
    elseif pState.phase == "COMBAT" then
        if action == "FIGHT" then
            handleFight(player, pState)
        elseif action == "RUN" then
            handleRun(player, pState)
        elseif action == "THROW_CARGO" then
            handleThrowCargo(player, pState, data.goodIndex, data.quantity)
        end
    elseif pState.phase == "EVENT" then
        if action == "DIALOG_RESPONSE" then
            handleDialogResponse(player, pState, data.dialogType, data.response)
        end
    elseif pState.phase == "BANKRUPTCY" then
        if action == "ACCEPT_LOAN" then
            handleAcceptLoan(player, pState)
        elseif action == "REFUSE_LOAN" then
            handleRefuseLoan(player, pState) -- game over
        end
    end

    -- After every action, push updated state to client
    StateUpdateEvent:FireClient(player, pState.gameState, pState.phase)
end)
```
...
**Action button visibility must follow the menu rules from section 2.2:**
- **Not in Hong Kong:** Show Buy, Sell, Quit only
- **In HK, net worth < 1,000,000:** Show Buy, Sell, Quit, Transfer, Visit Wu
- **In HK, net worth >= 1,000,000:** Show Buy, Sell, Quit, Transfer, Visit Wu, Retire
- The Warehouse panel should only be visible when in Hong Kong
- Net worth = cash + bank - debt, recalculated on each state update
...
StarterGui/
  MainScreenGui/
    HUDFrame/
      ...
      FinanceBar
      MarketPricesPanel
      ActionButtonsFrame      -- dynamically shows/hides per menu rules
      MessageLog
    CombatFrame/
      CombatHeader
      EnemyShipGrid
...
src/
  server/
    ...
    DataPersistence.lua       -- DataStoreService save/load with retry logic
...
local Remotes = {
    ...
    RequestStateUpdate = RemoteEvent, -- () -> triggers server to push StateUpdate
...
**Security note: No RemoteFunctions.** The original v2.0 design included a `GetGameState` RemoteFunction. This has been removed because `RemoteFunction` callbacks are vulnerable to client-side yield attacks: a malicious client can yield indefinitely in the callback, blocking the server thread that invoked it. Instead:
- The server **proactively pushes** state updates to the client via `StateUpdate` (RemoteEvent) after every action.
- If the client needs a fresh state (e.g., after reconnection), it fires `RequestStateUpdate` (RemoteEvent), which triggers the server to fire `StateUpdate` back to that specific player. This is a fire-and-forget pattern with no server-side yield risk.
...
- Grid position formula: slot I maps to X = `(I mod 5) * 8 + 1`, Y = `INT(I/5) * 6 + 7` (in original screen coordinates; translate to Roblox 3D or UI coordinates proportionally)
...
- New ships spawn into empty grid slots using the vacancy algorithm (see section 2.6)
...
| `RemoteEvent` | Client-server communication (used exclusively; no RemoteFunctions) |
| `TextService` | Text filtering for firm name input |
| `Players.PlayerRemoving` | Auto-save on disconnect + cleanup per-player state |
...
-- Retry wrapper with exponential backoff for all DataStore operations.
-- DataStore calls can fail transiently due to rate limits or network issues.
-- Note: BindToClose has ~30 second budget, so retry delays must be short.
local function dataStoreRetry(fn)
    for attempt = 1, 3 do
    ...
    warn("DataStore operation failed after 3 attempts")
    return false, nil
end
...
local function saveGame(player: Player, state: GameState): boolean
    ...
    if not success then
        warn("Failed to save game for", player.Name)
    end
    return success
end
...
- Roblox allows approximately `60 + 10 * playerCount` SetAsync calls per minute per DataStore.
- For a game with few concurrent players, this budget is generous, but saves should still be batched sensibly.
- **Recommended save strategy:** Save only on significant events — port departure, port arrival, retirement, game over, and player disconnect. Do NOT save after every sub-event (individual buy/sell, each combat round, etc.).
- Alternative: use a "dirty flag" approach — mark the game state as dirty when it changes, and persist at the next natural save point (port departure or arrival). This ensures no save budget is wasted on intermediate states.

**Save triggers:**
- Arriving at any port (auto-save)
- Before player disconnects (`Players.PlayerRemoving`)
- **On server shutdown (`game:BindToClose()`)** — essential for Roblox, as the server may shut down without individual PlayerRemoving events firing (e.g., when the last player leaves and the server closes, or during Roblox maintenance). The `BindToClose` handler should iterate all active players and save their states. Note the ~30 second budget: with retry delays of 2s and 4s, each player save can take up to ~6 seconds worst-case, so plan accordingly for server population.
...
**Mid-combat saves:**
For v1, it is recommended to **not** save mid-combat due to the complexity of serializing combat state. Instead, save at combat resolution (win, flee, death). If mid-combat saves are added in a future version, the following combat state must be serialized:
- `AM%` array: 10 slots, each with HP and damage (20 values)
- Scalar variables: `SN`, `SA`, `SS`, `OK`, `IK`, `RF`, `F1`, `S0`, `BT`
- The player's last command (`LC`) and current command (`CMD`)
...
- Each player gets their own independent `GameState` instance on the server, stored in the `playerStates` map
- Multiple players can be in the same Roblox server but are playing completely separate games
- PlayerAdded initializes per-player state; PlayerRemoving saves and cleans it up
...
- Use `math.floor(x + 0.5)` where the original uses `INT(x + .5)` for rounding (ship status, ship repair)
...
local function executeCombatRound(player, pState, action, data)
    -- 1. Resolve player action (server-side)
    local result = resolvePlayerAction(pState, action, data)

    -- 2. Send result to client for animation
    CombatUpdate:FireClient(player, result, pState.combatState)
    ...
    if pState.combatState.totalEnemies > 0 then
        local enemyResult = resolveEnemyFire(pState)
        CombatUpdate:FireClient(player, enemyResult, pState.combatState)
        waitForClientAck(player, 5)
    end

    -- 5. Check end conditions
    checkCombatEnd(player, pState)
end
```
...
**Combat targeting implementation:** When firing guns, each shot picks a random grid slot and retries if empty. Ensure the `SN > 0` guard is checked before entering the targeting loop to prevent infinite retries on an empty grid:

```lua
local function fireGuns(pState)
    local combat = pState.combatState
    local sunk = 0
    for gun = 1, pState.gameState.guns do
        if combat.totalEnemies <= 0 then break end

        -- Slot-retry targeting (lines 5320-5330)
        local slot
        repeat
            slot = fnR(10)
        until combat.onScreenEnemies[slot] and combat.onScreenEnemies[slot].hp > 0

        -- Apply damage
        local damage = fnR(30) + 10
        local enemy = combat.onScreenEnemies[slot]
        enemy.damage = enemy.damage + damage
        if enemy.damage > enemy.hp then
            -- Ship sunk — clear slot, spawn replacement if waiting
            enemy.hp = 0
            enemy.damage = 0
            sunk = sunk + 1
            combat.totalEnemies = combat.totalEnemies - 1
            combat.onScreenCount = combat.onScreenCount - 1
            spawnReplacementIfWaiting(combat, slot)
        end
    end
    return sunk
end
...
- The initial game start (D=0) skips time/financial processing entirely
- Non-HK ports skip directly to ship upgrade/gun checks
...
```lua
local function processArrival(player, pState, port: PortId)
    local state = pState.gameState
    -- ARRIVAL phase: non-interactive processing
    pState.phase = "ARRIVAL"

    -- D=0 guard: skip time/financial processing on initial game start (line 1010)
    if state.destination ~= 0 then
        advanceTime(state)
        compoundInterest(state)
        compoundDebt(state)
        checkJanuaryUpdates(state)
    end

    -- Build event queue (ordered per section 2.10)
    local events = {}

    -- Non-HK ports skip directly to ship upgrade checks (line 1030)
    if port == 1 then -- Hong Kong
        -- Interactive events (will switch to EVENT phase)
        checkLiYuenProtectionOffer(state, events)  -- line 1040
        checkShipRepair(state, events)              -- line 1120
        checkWuWarning(state, events)               -- line 1210 (narrative, non-interactive)
        checkWuVisit(state, events)                 -- line 1310 (interactive: debt/borrow)
        checkWuEnforcers(state, events)             -- line 1460 (after Wu visit)
    end

    checkShipUpgrade(state, events)                 -- line 1610
    checkGunPurchase(state, events)                 -- line 1710

    -- Non-interactive events (ARRIVAL phase continues)
    checkOpiumSeizure(state, events)                -- line 1900 (frees hold space)
    checkCargoTheft(state, events)                  -- line 2000 (updates WS)
    calculatePrices(state)                          -- line 2100
    checkLiYuenLapse(state, events)                 -- line 2310
    checkLiYuenWarning(state, events)               -- line 2330 (3-in-4 chance)
    checkPriceEvent(state, events)                  -- line 2410
    checkCashRobbery(state, events)                 -- line 2500
    ...
    for _, event in ipairs(events) do
        ...
        local response = presentEventToPlayer(player, event) -- async for interactive events
        resolveEvent(state, event, response)
    end

    -- Check for bankruptcy
    if isPlayerBroke(state) then
        pState.phase = "BANKRUPTCY"
        handleBankruptcy(player, pState)
        return
    end

    pState.phase = "PORT"
    StateUpdateEvent:FireClient(player, state, pState.phase)
end
```
...
5. **No RemoteFunctions:** Use only RemoteEvents to prevent client-side yield attacks (see section 3.6)

```lua
local function handleBuy(player, pState, goodIndex, quantity)
    -- Validate phase
    if pState.phase ~= "PORT" then return end
...
- **For v1, do not save mid-combat** — save at combat resolution (win, flee, retreat, death) instead. Mid-combat saves require serializing the full combat grid state (10 slots x 2 values in the AM% array, plus SN, SA, SS, OK, IK, RF, F1, S0, BT), adding significant complexity. This can be added in a future version if needed.
- On reconnect, restore the exact game phase (port state or post-combat)
...
- See section 4.4 for DataStore rate limit considerations — save at significant events, not every sub-action
...
| `D` | `destination` | PortId | Travel destination port (0 at game start) |
...
| `LI%` | — | — | Confirmed dead code: never initialized to non-zero, increment path unreachable |
...
| `WS` | `warehouseUsed` | number | Current warehouse usage (updated on theft and transfers) |
| `RF` | `raftFactor` | number | Accumulated from thrown cargo (not read elsewhere — vestigial) |
...
| `OK` | `combatResult` / `runMomentum` | number | Run momentum during combat; outcome code after (0=sunk, 1=won, 2=Li Yuen saved, 3=fled, 16=millionaire retirement) |
