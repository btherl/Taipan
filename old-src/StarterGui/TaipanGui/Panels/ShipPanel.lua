...
local function label(parent, name, size, pos, text, color, textSize, zIdx)
  ...
  lbl.BackgroundTransparency = 1; lbl.TextColor3 = color or AMBER
  lbl.Font = Enum.Font.RobotoMono; lbl.TextSize = textSize or 14
  lbl.TextXAlignment = Enum.TextXAlignment.Left
  lbl.TextWrapped = true; lbl.ZIndex = zIdx or 12
  lbl.Text = text or ""; lbl.Parent = parent
  return lbl
end
...
local function button(parent, name, size, pos, text, bgColor, fgColor, zIdx)
  ...
  btn.BackgroundColor3 = bgColor; btn.BorderColor3 = BORDER
  btn.TextColor3 = fgColor; btn.Font = Enum.Font.RobotoMono
  btn.TextSize = 14; btn.ZIndex = zIdx or 12
  btn.Text = text; btn.Parent = parent
  return btn
end

function ShipPanel.new(parent, onRepair, onAcceptUpgrade, onDeclineUpgrade, onAcceptGun, onDeclineGun, onDone)
  ...
  frame.BorderSizePixel = 0; frame.ZIndex = 12
  frame.Visible = false; frame.Parent = parent

  local y = 10
  label(frame, "Title", UDim2.new(1, -10, 0, 30), UDim2.new(0, 5, 0, y),
    "[ HONG KONG — SHIP OFFERS ]", AMBER, 16)
  y = y + 36

  -- Upgrade section (100px sub-frame)
  ...
  upgradeFrame.BorderColor3 = BORDER; upgradeFrame.ZIndex = 12
  upgradeFrame.Visible = false; upgradeFrame.Parent = frame
  ...
  gunFrame.BorderColor3 = Color3.fromRGB(60, 60, 110); gunFrame.ZIndex = 12
  gunFrame.Visible = false; gunFrame.Parent = frame
  ...
  repairFrame.BorderColor3 = Color3.fromRGB(110, 60, 30); repairFrame.ZIndex = 12
  repairFrame.Visible = false; repairFrame.Parent = frame
  ...
  amountBox.TextColor3 = AMBER; amountBox.Font = Enum.Font.RobotoMono
  amountBox.TextSize = 14; amountBox.PlaceholderText = "amount"
  amountBox.ZIndex = 12; amountBox.Parent = repairFrame
  ...
  allBtn.Activated:Connect(function() amountBox.Text = tostring(currentAllCost) end)
  repairBtn.Activated:Connect(function()
    local amt = tonumber(amountBox.Text)
    if amt and amt > 0 then onRepair(math.floor(amt)); amountBox.Text = "" end
  end)
  y = y + 88
  ...
  function panel.update(state)
    ...
    if offer.upgradeOffer then
      ...
      upgradeLabel.Text = string.format(
        "A merchant offers +50 tons capacity (fully repaired) for $%d!", offer.upgradeOffer.cost)
    else upgradeFrame.Visible = false end

    if offer.gunOffer then
      gunFrame.Visible = true
      gunLabel.Text = string.format(
        "An arms dealer offers a cannon for $%d (needs 10 tons hold).", offer.gunOffer.cost)
    else gunFrame.Visible = false end

    if offer.repairRate and state.damage > 0 then
      repairFrame.Visible = true
      local br = offer.repairRate
      local sw = math.max(0, 100 - math.floor(state.damage / state.shipCapacity * 100))
      currentAllCost = br * math.ceil(state.damage) + 1
      repairLabel.Text = string.format(
        "Ship Repair: $%d/unit  |  SW: %d%%  |  Damage: %d", br, sw, math.floor(state.damage))
    else repairFrame.Visible = false; currentAllCost = 0 end
  end
  return panel
end
